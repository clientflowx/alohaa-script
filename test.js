...
function triggerEveryThing() {
        ...
        if (btnSettings) {
            ...
            btnSettings.addEventListener('click', () => {
                // if (window.locationUserId === "FQ2QW4FaaR2EA12eaHnC") {
                    console.log("called to add event listener to integration btn for alloha")
        ...
        const integrationBtnTimer = setInterval(() => {
            ...
            if (btnIntegration) {
                ...
                btnIntegration.addEventListener('click', () => {
                    console.log("window.locationUserId", window.locationUserId, window.location.href)
                    // if (window.locationUserId === "FQ2QW4FaaR2EA12eaHnC") {
    ...
    const insertManageAllohaTab = () => {
        console.log("123444")
        setTimeout(() => {
            const outer = document.getElementsByClassName("hl_settings--body hl_v2_stettings--body hl_without-topbar")
    ...
    var manageAllohaPageDesiredUrlPattern = /^https:\/\/app\.clientflowx\.com\/v2\/location\/[A-Za-z0-9]+\/settings\/integrations\/list$/;
    ...
    const findMobileNumberOfCurrentUser = () => {
        ...
        const mobileNumberOfUser = document.getElementsByClassName("truncate text-sm font-normal text-gray-600 hover:text-primary-700 cursor-pointer")[0]?.innerHTML
        console.log("mobile===", mobileNumberOfUser)
        if (mobileNumberOfUser) {
            const formattedNumber = mobileNumberOfUser.replace(/^(\+91\s*)/g, '').replace(/\s/g, '');
    ...
    var callAllohaPageDesiredUrlPattern = /https:\/\/app\.clientflowx\.com\/v2\/location\/[A-Za-z0-9]+\/conversations\/conversations\/[A-Za-z0-9]+/;
    ...
    function addEventListenerToAllohaCallBtn() {
        ...
        allohaCallBtn.addEventListener("click", async () => {
            ...
            try {
                ...
                const response = await fetch(`${apiUrl}/api/customer/alloha-make-call`, config);
        ...
        const timeId = setInterval(() => {
            ...
            if (existingDiv) {
                ...
                if (buttonGroupDiv) {
                    ...
                    const secondButton = document.querySelector('.button-group button:nth-child(1)');
                    ...
                    newButton.classList.add('alloha-call-btn', 'flex', 'inline-flex', 'items-center', 'px-2.5', 'py-1', 'border', 'border-gray-300', 'rounded-l-md');
                    newButton.style.paddingLeft = '15px';
                    // newButton.style.background = "gray";

                    // Set button content
                    newButton.innerHTML = '<span><img src="https://res.cloudinary.com/dtqzhg98l/image/upload/v1705251766/Alohaa_orqfaa.png" style="height:20px;"/></span>'; // You can customize the text
    ...
    const callInfoAllohaPageDesiredUrlPattern = /^https:\/\/app\.clientflowx\.com\/v2\/location\/[A-Za-z0-9]+\/reporting\/call_stats$/;
